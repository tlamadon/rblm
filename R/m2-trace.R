...
m2.trace.simulate <- function(model) {
  ...
  sdata = sdata[,list(y1 = psi1 + rnorm(ceiling(size))*model$eps_sd/sqrt(2)), list(f1,size,psi1) ]
...
m2.firmfe.pen  <- function(sim, model, lambda=1) {
  
  # index firms with integers
  fids = sim$sdata[, ] data.table(f1=f1s,nfid=1:length(f1s))
  setkey(fids,f1)
  setkey(jdata,f1)
  jdata[,f1i := fids[jdata,nfid]]
  setkey(jdata,f2)
  ...
  # create the penalty, attach the cluster mean
  grp       = unique(sim$sdata[,list(f1,j1)])
  setkey(grp,f1)
  setkey(fids,f1)
  fids[, ]  

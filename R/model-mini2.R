...
model.mini2.new <- function(nf,linear=FALSE,serial=F) {
  ...
  m$A1 = scale(cumsum(abs(rnorm(nf))),scale = FALSE)
  m$B1 = exp(rnorm(nf)/2)
  m$A2 = rnorm(nf)
  m$B2 = exp(rnorm(nf)/2)
...
model.mini2.liml.int2 <- function(Y1,Y2,J1,J2,norm=1) {
  ...
  Wz = t(R) %*% R  -  ( Matrix:::t(R) %*% Z)  %*% solve( Matrix:::t(Z)  %*% Z  ) %*% ( Matrix:::t(Z) %*% R)
  Wx = t(R) %*% R  -  ( Matrix:::t(R) %*% X2) %*% solve( Matrix:::t(X2) %*% X2 ) %*% ( Matrix:::t(X2) %*% R)
  ...
  XX = cBind(X1,X2)
  RR = (1-lambda)*Matrix:::t(XX) %*% XX + lambda * ( Matrix:::t(XX) %*% Z)  %*% solve( Matrix:::t(Z)  %*% Z  ) %*% ( Matrix:::t(Z) %*% XX)
  RY = (1-lambda)*Matrix:::t(XX) %*% Y  + lambda * ( Matrix:::t(XX) %*% Z)  %*% solve( Matrix:::t(Z)  %*% Z  ) %*% ( Matrix:::t(Z) %*% Y)
...
model.mini2.estimate <- function(jdata,sdata,norm=1,model0=c()) {
  ...
  nf = max(J1)
  rliml = model.mini2.liml.int2(Y1,Y2,J1,J2,norm=norm)
  B1 = rliml$B1
  B2 = rliml$B2
  N = length(Y1)
